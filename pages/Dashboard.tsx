...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-end gap-6 text-emerald-950">
        ...
        <div className="flex items-center gap-4 glass p-4 rounded-2xl shadow-sm border border-emerald-100">
          <div className="w-12 h-12 bg-emerald-600 text-white flex items-center justify-center rounded-xl font-black text-xl shadow-lg shadow-emerald-100">
            {stats.ecoLevel[0]}
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-[0.2em] text-emerald-400 font-black">Rank Level</div>
            <div className="font-black text-emerald-950 tracking-tight">{stats.ecoLevel}</div>
          </div>
        </div>
      </header>

      {/* Grid Matrix */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-emerald-950">
        <div className="col-span-1 md:col-span-2 glass p-8 rounded-[2.5rem] relative overflow-hidden group border border-emerald-100 shadow-sm">
        ...
        <div className="glass p-8 rounded-[2.5rem] flex flex-col justify-between hover:border-emerald-300 transition-all cursor-pointer border border-emerald-100 shadow-sm">
          <ICONS.Leaf className="w-8 h-8 text-emerald-500" />
          <div>
        ...
        <div className="glass p-8 rounded-[2.5rem] flex flex-col justify-between border border-emerald-200 bg-emerald-50 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Trend Graph */}
        <div className="lg:col-span-2 glass p-8 rounded-[2.5rem] h-[450px] border border-emerald-100 shadow-sm">
          <h3 className="text-xl font-bold mb-8 serif italic text-emerald-950">Performance Index</h3>
        ...
        {/* List Matrix */}
        <div className="glass p-8 rounded-[2.5rem] flex flex-col text-emerald-950 border border-emerald-100 shadow-sm">
          <h3 className="text-xl font-bold mb-8 serif italic">Recent Audits</h3>
          <div className="space-y-6 flex-grow">
            {history.length > 0 ? history.slice(0, 5).map((item) => (
              <div key={item.id} className="flex items-center justify-between group cursor-pointer border-b border-emerald-500/5 pb-4">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 glass border border-emerald-100 flex items-center justify-center rounded-xl text-xs font-black group-hover:bg-emerald-600 group-hover:text-white transition-all shadow-sm">
                    {item.generalScore}
                  </div>
                  <div>
                    <div className="text-sm font-bold truncate max-w-[120px] text-emerald-950">{item.productName}</div>
          ...
          {history.length > 0 && (
            <button className="mt-8 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 hover:text-emerald-700 transition-colors">
              View All History →
            </button>
          )}
        </div>
